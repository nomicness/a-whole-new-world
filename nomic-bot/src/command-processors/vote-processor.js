...
const voteProcessor = {
    ...
    processVote: function (commentsUrl, userLogin, requestBody) {
        ...
        const labelsUrl = requestBody.issue.labels_url, issueTitle = requestBody.issue.title, issueOrdinal = voteProcessor.expressions.ordinal.exec(issueTitle)[1], issueNumber = requestBody.issue.number, currentLabels = requestBody.issue.labels;
        ...
        .then(function (comments) {
            return github.getPlayerData()
                .then(function (playerData) {
                    let activePlayers = playerData.activePlayers, votes = [], hasQuorum = false, voteValue = 0, labels = [voteProcessor.labelTitles.open], quorum = Math.ceil(activePlayers.length * 0.5), processedUsers = [], pendingUsers = [];
