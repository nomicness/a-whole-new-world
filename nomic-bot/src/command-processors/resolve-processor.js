...
const resolveProcessor = {
    ...
    processPoints: function (playerData, player, issue) {
        const ordinal = Number(voteProcessor.expressions.ordinal.exec(issue.title)[1]), value = ordinal - 291, message = stringFormat(resolveProcessor.messages.pointValue, {login: player.name, ordinal: ordinal, issueNumber: issue.number, value: value});
    ...
    processResolve: function (commentsUrl, userLogin, requestBody) {
        ...
        .then(function (playerData) {
            ...
            const activePlayers = playerData.activePlayers, player = _.find(activePlayers, {name: userLogin});
