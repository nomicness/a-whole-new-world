...
const rollProcessor = {
    ...
    processRoll: function (commentsUrl, userLogin, requestBody) {
        ...
        let baseExpression = /([0-9]+)\s*[dD]\s*([0-9]+)/,
    ...
    simple: function (comment, response) {
        ...
        const rollInstruction = rollProcessor.expressions.simple.exec(comment), dieCount = Number(rollInstruction[1]), sides = Number(rollInstruction[2]);
    ...
    sum: function (comment, response) {
        ...
        const rollInstruction = rollProcessor.expressions.sum.exec(comment), dieCount = Number(rollInstruction[1]), sides = Number(rollInstruction[2]), add = Number(rollInstruction[3]);
    ...
    subtraction: function (comment, response) {
        ...
        const rollInstruction = rollProcessor.expressions.sum.exec(comment), dieCount = Number(rollInstruction[1]), sides = Number(rollInstruction[2]), sub = Number(rollInstruction[3]);
