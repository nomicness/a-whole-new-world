...
const hungerProcessor = {
    ...
    reducePopulation: function (player, amount) {
        ...
        _.times(amount, function () {
            const index = _.random(0, keys.length - 1), key = keys[index];
    ...
    getActiveFarms: function (player, farmCount) {
        const farms = farmCount || _.get(player, 'village.farms', 0), farmers = _.get(player, 'village.population.farming', 0);
    ...
    processFarmProduction: function (player, farms) {
        const activeFarms = hungerProcessor.getActiveFarms(player, farms), production = _.sum(_.times(activeFarms, _.partial(rollProcessor.sum, hungerProcessor.farmProduction, {}))), currentHunger = _.get(player, 'village.hunger', 0);
    ...
    processPlayerHunger: function (playerData, player) {
        ...
        const production = hungerProcessor.processFarmProduction(player), populationCount = hungerProcessor.getTotalPopulation(player);
